...
namespace HillEngine
{
	...
	EventManager::EventManager() : keyBindings(NULL), numKeyBindings(0)
	{
		...
		lightList = new Light**[LIGHTTYPE_NUM];
		lightNum = new int[LIGHTTYPE_NUM];
		for (int i = 0; i < LIGHTTYPE_NUM; i++)
		{
			lightList[i] = new Light*[0];
			lightNum[i] = 0;
		}
	}
	...
	void EventManager::mainLoop()
	{
		...
		GLuint deferredShaders[2];

		// Loads deferred shading shaders
		deferredShaders[LIGHTTYPE_POINT] = loadShaders("quadVertex.glsl", "deferredFragmentPoint.glsl");
		GLuint positionPointTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "position_cameraspaceTex");
		GLuint diffusePointTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "diffuseTex");
		GLuint specularPointTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "specularTex");
		GLuint normalPointTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "normalTex");
		GLuint lightPointPositionId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "lightPosition_cameraspace");
		GLuint lightPointColorId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "lightColor");
		GLuint lightPointPowerId = glGetUniformLocation(deferredShaders[LIGHTTYPE_POINT], "lightPower");

		deferredShaders[LIGHTTYPE_DIRECTIONAL] = loadShaders("quadVertex.glsl", "deferredFragmentDirectional.glsl");
		GLuint positionDirectionalTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "position_cameraspaceTex");
		GLuint diffuseDirectionalTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "diffuseTex");
		GLuint specularDirectionalTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "specularTex");
		GLuint normalDirectionalTexId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "normalTex");
		GLuint lightDirectionDirectionalId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "lightDirection_cameraspace");
		GLuint lightDirectionalColorId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "lightColor");
		GLuint lightDirectionalPowerId = glGetUniformLocation(deferredShaders[LIGHTTYPE_DIRECTIONAL], "lightPower");
		...
		while (cont)
		{
			...
			// Load textures that will be necessary for all lights
			...
			for (int i = 0; i < LIGHTTYPE_NUM; i++)
			{
				...
				case LIGHTTYPE_POINT:
					glUseProgram(deferredShaders[LIGHTTYPE_POINT]);

					glUniform1i(positionPointTexId, 0);
					glUniform1i(diffusePointTexId, 1);
					glUniform1i(specularPointTexId, 2);
					glUniform1i(normalPointTexId, 3);

					// Render for each light
					for (int j = 0; j < lightNum[i]; ++j)
					{
						glm::vec4 lightPosition_cameraspace = viewMatrix * vec4(lightList[i][j]->location, 1);
						glUniform3f(lightPointPositionId, lightPosition_cameraspace.x, lightPosition_cameraspace.y, lightPosition_cameraspace.z);
						vec3 lightCol = lightList[i][j]->getColor();
						glUniform3f(lightPointColorId, lightCol.r, lightCol.g, lightCol.b);
						glUniform1f(lightPointPowerId, lightList[i][j]->getPower());
				...
				case LIGHTTYPE_DIRECTIONAL:
					glUseProgram(deferredShaders[LIGHTTYPE_DIRECTIONAL]);

					glUniform1i(positionDirectionalTexId, 0);
					glUniform1i(diffuseDirectionalTexId, 1);
					glUniform1i(specularDirectionalTexId, 2);
					glUniform1i(normalDirectionalTexId, 3);

					// Render each directional light
					for (int j = 0; j < lightNum[i]; ++j)
					{
						glm::vec4 lightDirection_cameraspace = viewMatrix * (lightList[i][j]->rotation * vec4(0, 1, 0, 0));
						glUniform3f(lightDirectionDirectionalId, lightDirection_cameraspace.x, lightDirection_cameraspace.y, lightDirection_cameraspace.z);
						vec3 lightCol = lightList[i][j]->getColor();
						glUniform3f(lightDirectionalColorId, lightCol.r, lightCol.g, lightCol.b);
						glUniform1f(lightDirectionalPowerId, lightList[i][j]->getPower());
		...
		glDeleteProgram(deferredShaders[0]);
		glDeleteProgram(deferredShaders[1]);
	...
	{
		// Adds a new light
		Light** newLightList = new Light*[lightNum[newLight->getLightType()] + 1];
		memcpy(newLightList, lightList[newLight->getLightType()], sizeof(Light*)* lightNum[newLight->getLightType()]);
		newLightList[lightNum[newLight->getLightType()]] = newLight;

		delete[] lightList[newLight->getLightType()];
		lightList[newLight->getLightType()] = newLightList;

		lightNum[newLight->getLightType()]++;
	}
